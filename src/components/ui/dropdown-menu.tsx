...
>(({ className, ...props }, ref) => (
  <DropdownMenuPrimitive.SubContent
    ref={ref}
    className={cn(
      "z-50 min-w-[8rem] overflow-hidden rounded-md border bg-white p-1 text-slate-950 shadow-md animate-in slide-in-from-left-1",
      className
    )}
    {...props}
  />
...
>(({ className, sideOffset = 4, ...props }, ref) => (
  <DropdownMenuPortal>
    <DropdownMenuPrimitive.Content
      ...
      className={cn(
        "z-50 min-w-[12rem] overflow-hidden rounded-md border bg-white p-1 text-slate-950 shadow-md focus:outline-none animate-in fade-in-0 zoom-in-95",
        className
      )}
      {...props}
    />
...
>(({ className, children, checked, ...props }, ref) => (
  ...
  >
    <span className="absolute left-2 flex h-3.5 w-3.5 items-center justify-center">
      {checked && <span className="h-2 w-2 rounded-sm bg-slate-900" />}
    </span>
    {children}
  </DropdownMenuPrimitive.CheckboxItem>
))
